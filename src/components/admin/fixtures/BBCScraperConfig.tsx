...
const BBCScraperConfig: React.FC = () => {
  ...
  // Save configuration to Supabase settings
  ...
  const handleRunScraper = async () => {
    ...
    try {
      ...
      if (data?.success) {
        toast.success(`Scraper executed successfully, found ${data.fixtures?.length || 0} fixtures`);
      } else {
        throw new Error(data?.message || 'Unknown error');
      }
    } catch (error) {
      console.error('Error running scraper:', error);
      toast.error(`Failed to run scraper: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsLoading(false);
    }
  };
...
export default BBCScraperConfig;
