...
export class FirecrawlService {
  ...
  static async scrapeHighlandLeagueFixtures(): Promise<{ success: boolean; data?: ScrapedFixture[]; error?: string }> {
    try {
      ...
      // Call our Supabase Edge Function to handle the scraping
      const { data, error } = await fetch('/api/scrape-fixtures', {
        method: 'POST',
        headers: {
      ...
      console.error('Error in scrapeHighlandLeagueFixtures:', error);
      return { 
        success: false, 
        error: error instanceof Error ? error.message : 'Failed to scrape fixtures'
      };
    }
  }
}
